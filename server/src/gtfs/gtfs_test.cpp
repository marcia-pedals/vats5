...
GTEST("GtfsLoad should work") {
  ...
  EXPECT_THAT(
      ...
      Contains(AllOf(
          Field(
              &GtfsTrip::route_direction_id,
              AllOf(
                  Field(
                      &GtfsRouteDirectionId::route_id,
  ...
  EXPECT_THAT(
      ...
      Contains(AllOf(
          Field(
              &GtfsDirection::route_direction_id,
              AllOf(
                  Field(
                      &GtfsRouteDirectionId::route_id,
                      Field(&GtfsRouteId::v, Eq("SR:3"))
...
GTEST("GtfsSave round-trip should preserve data") {
  ...
  EXPECT_EQ(
      original_gtfs_day.stop_times.size(), reloaded_gtfs.stop_times.size()
  );
  EXPECT_EQ(original_gtfs_day.routes.size(), reloaded_gtfs.routes.size());
  EXPECT_EQ(
      original_gtfs_day.directions.size(), reloaded_gtfs.directions.size()
  );
...
GTEST("GtfsFilterByTrips should filter by specific trip") {
  ...
  std::vector<GtfsStopTime> expected_stop_times = {
      GtfsStopTime{
          trip_id,
          san_jose_diridon_northbound,
          1,
          ParseGtfsTime("07:22:00"),
          ParseGtfsTime("07:22:00")
      },
      GtfsStopTime{
          trip_id,
          sunnyvale_northbound,
          2,
      ...
      GtfsStopTime{
          trip_id,
          mountain_view_northbound,
          3,
          ParseGtfsTime("07:36:00"),
          ParseGtfsTime("07:36:00")
      },
      GtfsStopTime{
          trip_id,
          palo_alto_northbound,
          4,
      ...
      GtfsStopTime{
          trip_id,
          redwood_city_northbound,
          5,
          ParseGtfsTime("07:49:00"),
          ParseGtfsTime("07:49:00")
      },
      GtfsStopTime{
          trip_id,
          hillsdale_northbound,
          6,
      ...
      GtfsStopTime{
          trip_id,
          sf_22nd_street_northbound,
          10,
          ParseGtfsTime("08:16:00"),
          ParseGtfsTime("08:16:00")
      },
      GtfsStopTime{
          trip_id,
          sf_4th_king_northbound,
          11,
          ParseGtfsTime("08:22:00"),
          ParseGtfsTime("08:22:00")
      }
  };
...
auto StopMatcher(const std::string& stop_id, const std::string& stop_name) {
  return AllOf(
      Field(&GtfsStop::stop_id, Field(&GtfsStopId::v, Eq(stop_id))),
...
GTEST("GtfsNormalizeStops should replace child stops with parents") {
  ...
  EXPECT_THAT(
      ...
      UnorderedElementsAre(
          ...
          StopMatcher(
              "mtc:warm-springs-south-fremont-bart",
              "Warm Springs South Fremont BART"
          ),
          StopMatcher(
              "mtc:el-cerrito-del-norte-bart", "El Cerrito Del Norte BART"
          ),
          ...
          StopMatcher(
              "mtc:12th-st-oakland-city-center-bart",
          ...
          StopMatcher(
              "mtc:great-mall-milpitas-bart", "Great Mall/Milpitas BART"
          ),
