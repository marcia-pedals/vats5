...
ProblemState EliminateEdges(const ProblemState& base_state, int threshold) {
  ...
  auto worker = [&]() {
    while (true) {
      ...
      bool eliminated =
          !lb_result.has_value() || lb_result->optimal_value >= threshold;
  ...
  std::cout << "Processing " << total << " edges with " << num_threads
            << " threads (threshold "
            << TimeSinceServiceStart{threshold}.ToString() << "):\n";
...
void EliminateIntermediateStops(ProblemState& state, int threshold) {
      ...
      if (is_eliminated) {
        ...
        for (const Step& step : state.minimal.AllSteps()) {
          if (step.origin.stop == stop || step.destination.stop == stop) {
            forbids.push_back(
                BranchEdge{step.origin.stop, step.destination.stop}.Forbid()
            );
          }
        }
        if (!forbids.empty()) {
  ...
  std::cout << "Processing " << total << " intermediate stops with "
            << num_threads << " threads (threshold "
            << TimeSinceServiceStart{threshold}.ToString() << "):\n";
  std::vector<std::thread> threads;
  for (int t = 0; t < num_threads; ++t) {
    threads.emplace_back(worker);
  ...
  std::cout << "Eliminated " << num_eliminated << " / " << total
            << " intermediate stops.\n";
}
