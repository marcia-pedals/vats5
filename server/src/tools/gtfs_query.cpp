...
void QueryRequiredStopsConfig(
  ...
  std::sort(sorted_stops.begin(), sorted_stops.end(),
            [](const GtfsStopId& a, const GtfsStopId& b) {
              return a.v < b.v;
            });

  // Output the TOML file
  std::cout << "# Generated by: gtfs_query " << config_path << " required_stops_config";
  if (!trip_id_prefix.empty()) {
    std::cout << " --trip_id_prefix " << trip_id_prefix;
  }
  if (!route_ids.empty()) {
    std::cout << " --route_ids " << route_ids;
  }
  std::cout << "\n\nstop_ids = [\n";
...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (command == "stops") {
      ...
      QueryRequiredStopsConfig(data, config_path, trip_id_prefix, route_ids);
    } else {
