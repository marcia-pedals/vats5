...
//
// If `preserve` is non-null, slack forwarding only guarantees not to weaken
// edges (origin state, destination state) for which it returns true; other
// edges may end up below their baseline weight, freeing their steps' slack to
// strengthen the preserved ones. The result is a valid lower-bound weight set
// either way.
