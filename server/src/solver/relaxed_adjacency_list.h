...
// Non-templated base class for RelaxedAdjacencyList<>.
struct RelaxedAdjacencyListBase {
  int space_id = -1;
  ...
  bool operator==(const RelaxedAdjacencyListBase& other) const {
    return edge_offsets == other.edge_offsets && edges == other.edges;
  }
};

// Templated subclass for RelaxedAdjacencyList<>.
template <typename Tag = Unwitnessed>
struct RelaxedAdjacencyList : RelaxedAdjacencyListBase {
  using RelaxedAdjacencyListBase::RelaxedAdjacencyListBase;
  using RelaxedAdjacencyListBase::GetEdges;
  using RelaxedAdjacencyListBase::GetWeight;

  template <typename S> requires CompatibleWith<S, Tag>
  std::span<const RelaxedEdge> GetEdges(StopId<S> stop) const
    requires (!std::same_as<Tag, Unwitnessed>)
  {
    return RelaxedAdjacencyListBase::GetEdges(StopId<>{stop.v});
  }

  template <typename S> requires CompatibleWith<S, Tag>
  std::optional<int> GetWeight(StopId<S> from, StopId<S> to) const
    requires (!std::same_as<Tag, Unwitnessed>)
  {
    return RelaxedAdjacencyListBase::GetWeight(
        StopId<>{from.v}, StopId<>{to.v});
  }
};
