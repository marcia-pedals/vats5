...
std::vector<Path> ComputeMinimalFeasiblePathsAlong(
  ...
  // Compute paths for the first edge on-demand.
  auto first_path_map =
      FindMinimalPathSet(minimal, stop_sequence[0], {stop_sequence[1]});
  auto first_it = first_path_map.find(stop_sequence[1]);
  if (first_it == first_path_map.end()) {
    return {};
  }
  std::vector<Path> paths = std::move(first_it->second);

  for (size_t i = 1; i + 1 < stop_sequence.size(); ++i) {
    auto next_path_map =
        FindMinimalPathSet(minimal, stop_sequence[i], {stop_sequence[i + 1]});
    auto next_it = next_path_map.find(stop_sequence[i + 1]);
    if (next_it == next_path_map.end()) {
      return {};
    }
    const std::vector<Path>& next_paths = next_it->second;
