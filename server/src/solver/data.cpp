...
StepsFromGtfs GetStepsFromGtfs(GtfsDay gtfs) {
    ...
    for (size_t i = 0; i < sorted_stop_times.size() - 1; ++i) {
        ...
        if (current_stop_time.trip_id == next_stop_time.trip_id) {
            StopId origin_stop_id = result.mapping.gtfs_stop_id_to_stop_id[current_stop_time.stop_id];
            StopId destination_stop_id = result.mapping.gtfs_stop_id_to_stop_id[next_stop_time.stop_id];
            TripId trip_id = result.mapping.gtfs_trip_id_to_trip_id[current_stop_time.trip_id];
